...
9. Task 12 では最初は `system` と `schema` のみ cli.py に登録し、Task 13-17 で各コマンドを実装するたびに cli.py への登録を追加する方式に変更
10. グローバルエラーハンドラに `Exception` フォールバック捕捉を追加（`FileNotFoundError` 等の非 `DavinciCLIError` 例外にも対応）
11. schema 登録の漏れを修正（timeline marker list/delete、clip select/property.get を追加）
12. schema 登録の `output_model` を各 `_impl` 関数の実際の戻り値と一致するよう修正:
    - `project.*` → コマンドごとに個別の Output モデルを定義（`ProjectInfo` 使い回しを解消）
      - `project.list` → `ProjectListItem`、`project.open` → `ProjectOpenOutput`、`project.close` → `ProjectCloseOutput`
      - `project.create` → `ProjectCreateOutput`、`project.delete` → `ProjectDeleteOutput`
      - `project.save` → `ProjectSaveOutput`、`project.info` → `ProjectInfoOutput`
      - `project.settings.get` → `ProjectSettingsGetOutput`、`project.settings.set` → `ProjectSettingsSetOutput`
    - `timeline.*` → コマンドごとに個別の Output モデルを定義（`TimelineInfo` 使い回しを解消）
      - `timeline.list` → `TimelineListItem`、`timeline.current` → `TimelineCurrentOutput`
      - `timeline.switch` → `TimelineSwitchOutput`、`timeline.create` → `TimelineCreateOutput`
      - `timeline.delete` → `TimelineDeleteOutput`、`timeline.export` → `TimelineExportOutput`
      - `timeline.marker.list` → `MarkerInfo`、`timeline.marker.add` → `MarkerAddOutput`
      - `timeline.marker.delete` → `MarkerDeleteOutput`
    - `clip.select` → `ClipSelectOutput`（`ClipInfo` から変更）
    - `clip.property.get` → `ClipPropertyGetOutput`（`ClipInfo` から変更）
    - `clip.property.set` → `ClipPropertySetOutput`（`ClipInfo` から変更）
13. `project.close`/`project.save`/`project.settings.get` の schema 登録を追加（漏れ修正）
14. `--fields` は表示フィルタ。schema は常にフルレスポンスの型を定義する（`--fields` 適用後の部分出力は schema 対象外。エージェントは `--fields` を使わずフル取得して自分でフィルタすべき）
...
class TestDavinciCLIGroup:
    ...
    def test_generic_exception_fallback(self):
        """DavinciCLIError 以外の例外も構造化エラーとして出力されること"""
...
class TestSubcommandRegistration:
    """Task 12 時点では system と schema のみ登録。
    project/timeline/clip は Task 13-17 で追加する。"""
...
class DavinciCLIGroup(click.Group):
    ...
    Exception のフォールバック捕捉も追加し、FileNotFoundError 等の
    非 DavinciCLIError 例外も構造化エラーメッセージとして出力する。
    トレースバックは出さない。
    """
    ...
    def invoke(self, ctx: click.Context) -> None:
        try:
            ...
            # フォールバック: DavinciCLIError 以外の例外も構造化エラーとして出力
            # FileNotFoundError 等がトレースバック表示されることを防ぐ
            error_response = {
                "error": str(exc),
                "error_type": type(exc).__name__,
                "exit_code": 1,
...
# Task 12 時点では system と schema のみ登録。
# Task 13-17 で各コマンドを実装するたびに、ここに登録を追加する。
def _register_commands() -> None:
    from davinci_cli.commands import system, schema
    dr.add_command(system.system)
    dr.add_command(schema.schema)
    # Task 15 で追加: from davinci_cli.commands import project
    # dr.add_command(project.project)
    # Task 16 で追加: from davinci_cli.commands import timeline
    # dr.add_command(timeline.timeline)
    # Task 17 で追加: from davinci_cli.commands import clip
...
注意: Task 12 時点では `_register_commands()` で system と schema のみ登録する。
commands/ 配下には空のスタブモジュールを作成する（system.py と schema.py は最低限の Click group 定義を含む）。
project/timeline/clip は Task 15-17 でそれぞれ実装する際に cli.py への登録も追加する。
...
- Modify: `src/davinci_cli/cli.py`（project コマンドを `_register_commands()` に追加）
...
class ProjectSettingsGetOutput(BaseModel):
    """project.settings.get の戻り値。"""
    key: str | None = None
    value: str | None = None
    settings: str | None = None

class ProjectSettingsSetOutput(BaseModel):
    """project.settings.set の戻り値。"""
    set: bool | None = None
    key: str | None = None
    value: str | None = None
    dry_run: bool | None = None
...
**Step 3.5: cli.py に project コマンドを登録**

`src/davinci_cli/cli.py` の `_register_commands()` に project を追加:
```python
def _register_commands() -> None:
    from davinci_cli.commands import system, schema, project
    dr.add_command(system.system)
    dr.add_command(schema.schema)
    dr.add_command(project.project)
```
...
```bash
git add src/davinci_cli/commands/project.py src/davinci_cli/cli.py tests/unit/test_project.py
git commit -m "feat: commands/project.py — 共通デコレータ使用、ProjectNotFoundError追加、cli.py登録"
```
...
- Modify: `src/davinci_cli/cli.py`（timeline コマンドを `_register_commands()` に追加）
...
class TimelineListItem(BaseModel):
    """timeline.list の戻り値の各要素。"""
    name: str
    fps: float | None = None

class TimelineCurrentOutput(BaseModel):
    """timeline.current の戻り値。"""
    name: str
    fps: float | None = None
    width: int | None = None
...
**Step 3.5: cli.py に timeline コマンドを登録**

`src/davinci_cli/cli.py` の `_register_commands()` に timeline を追加:
```python
def _register_commands() -> None:
    from davinci_cli.commands import system, schema, project, timeline
    dr.add_command(system.system)
    dr.add_command(schema.schema)
    dr.add_command(project.project)
    dr.add_command(timeline.timeline)
...
```bash
git add src/davinci_cli/commands/timeline.py src/davinci_cli/cli.py tests/unit/test_timeline.py
git commit -m "feat: commands/timeline.py — 共通デコレータ使用、マーカー操作含む、cli.py登録"
```
...
- Modify: `src/davinci_cli/cli.py`（clip コマンドを `_register_commands()` に追加）
...
class ClipPropertyGetOutput(BaseModel):
    """clip.property.get の戻り値。"""
    index: int
    key: str
    value: str | None = None

class ClipPropertySetOutput(BaseModel):
    """clip.property.set の戻り値。"""
    set: bool | None = None
    index: int | None = None
    key: str | None = None
    value: str | None = None
...
**Step 3.5: cli.py に clip コマンドを登録**

`src/davinci_cli/cli.py` の `_register_commands()` に clip を追加（最終形）:
```python
def _register_commands() -> None:
...
```bash
git add src/davinci_cli/commands/clip.py src/davinci_cli/cli.py tests/unit/test_clip.py
git commit -m "feat: commands/clip.py — 共通デコレータ使用、NDJSON出力対応、cli.py登録"
```
